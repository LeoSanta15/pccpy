...
import spyc
from spyc import control_chart_constants as cc
...
def test_imr_alternative_sigma_methods():
    mr = np.abs(np.diff(X))
    med = spyc.imr_chart(X, sigma_method="median_mr").params[0]["sigma"]
    assert med == pytest.approx(np.median(mr) / 0.954)
    mssd = spyc.imr_chart(X, sigma_method="mssd").params[0]["sigma"]
    assert mssd == pytest.approx(np.sqrt(np.sum(np.diff(X) ** 2) / (2 * (len(X) - 1))))
    with pytest.raises(ValueError):
        spyc.imr_chart(X, sigma_method="nope")
    with pytest.raises(ValueError):
        spyc.imr_chart(X, sigma_method="median_mr", span=3)


def test_imr_span_3():
    ch = spyc.imr_chart(X, span=3)
...
def test_imr_historical_parameters():
    ch = spyc.imr_chart(X, mu=50, sigma=2)
...
def test_default_is_test1_only_like_minitab(rng):
    ch = spyc.imr_chart(np.full(30, 5.0) + np.arange(30) * 0.0 + rng.normal(0, 1e-9, 30))
    assert ch.tests == (1,)
...
def test_stages_use_separate_limits_and_offset_indices(rng):
    ...
    x = np.r_[a, b]
    ch = spyc.imr_chart(x, stages=np.r_[np.ones(30), 2 * np.ones(30)], tests=[1])
    I = ch["I"]
    ...
    assert ch.params[0]["sigma"] < spyc.imr_chart(x).params[0]["sigma"]


def test_non_contiguous_stages_rejected():
    with pytest.raises(ValueError, match="contigua"):
        spyc.imr_chart(np.arange(6.0), stages=[1, 1, 2, 2, 1, 1])
    with pytest.raises(ValueError):
        spyc.imr_chart(np.arange(6.0), stages=[1, 2])


def test_missing_values_rejected():
    with pytest.raises(ValueError):
        spyc.imr_chart([1.0, np.nan, 3.0])


# ---------------------------------------------------------------- Xbar-R / Xbar-S
def test_xbar_r_matches_a2_d4():
    ch = spyc.xbar_r_chart(G)
    c = cc(5)
...
def test_pooled_sigma():
    ch = spyc.xbar_s_chart(G, sigma_method="pooled")
...
def test_subgroup_input_formats_are_equivalent():
    a = spyc.xbar_r_chart(G)
    b = spyc.xbar_r_chart(G.ravel(), subgroup_size=5)
    c = spyc.xbar_r_chart(G.ravel(), subgroup=np.repeat(np.arange(25), 5))
    for other in (b, c):
...
def test_unequal_subgroup_sizes_scale_limits_by_sqrt_n(rng):
    ...
    ch = spyc.xbar_r_chart(vals, subgroup=ids)
    xb = ch["Xbar"]
...
def test_wrong_method_for_chart():
    with pytest.raises(ValueError):
        spyc.xbar_r_chart(G, sigma_method="sbar")
    with pytest.raises(ValueError):
        spyc.xbar_s_chart(G, sigma_method="rbar")


def test_tests_1_to_8_apply_to_xbar_but_only_1_to_4_to_range():
    ch = spyc.xbar_r_chart(G, tests="all")
...
def test_attribute_validation():
    with pytest.raises(ValueError):
        spyc.np_chart(D, NV)  # n variable
    with pytest.raises(ValueError):
        spyc.p_chart([5, 200], 100)  # defectuosos > n
    with pytest.raises(ValueError):
        spyc.c_chart([1, -1, 2])
...
def test_p_chart_flags_outlier():
    d = D.copy()
    d[7] = 30
    assert 7 in spyc.p_chart(d, 100, tests=[1])["P"].violations[1]
...
def test_ewma_recursion_and_exact_limits(rng):
    ...
    ch = spyc.ewma_chart(x, weight=lam, target=10, sigma=s)
    z = ch["EWMA"]
    expect, prev = [], 10.0
    for v in x:
...
def test_ewma_detects_small_shift():
    ...
    ch = spyc.ewma_chart(x, target=0, sigma=1)
...
def test_cusum_hand_calculation():
    ...
    ch = spyc.cusum_chart(x, target=10, sigma=1, h=4, k=0.5)
    c = ch["CUSUM"]
...
def test_cusum_lower_side_and_subgroups():
    x = np.full(12, 9.0)
    c = spyc.cusum_chart(x, target=10, sigma=1)["CUSUM"]
    assert np.allclose(c.secondary, -0.5 * np.arange(1, 13))
    sub = spyc.cusum_chart(G, target=10, sigma=0.5)["CUSUM"]  # subgrupos de 5: sigma_x = 0.5/sqrt(5)
    assert sub.ucl[0] == pytest.approx(4 * 0.5 / np.sqrt(5))
    with pytest.raises(ValueError):
        spyc.ewma_chart(X, weight=0)
    with pytest.raises(ValueError):
        spyc.cusum_chart(X, h=-1)


# ---------------------------------------------------------------- resultado
def test_result_helpers():
    ch = spyc.imr_chart(X, tests="all")
    df = ch.to_frame()
