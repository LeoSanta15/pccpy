...
def test_individuals_match_hand_calculation():
    r = spyc.capability_analysis(X, LSL, USL, T)
...
def test_confidence_intervals_bracket_estimates():
    r = spyc.capability_analysis(X, LSL, USL, ci_level=0.95)
    assert r.pp_ci[0] < r.pp < r.pp_ci[1]
    assert r.ppk_ci[0] < r.ppk < r.ppk_ci[1]
    wide = spyc.capability_analysis(X, LSL, USL, ci_level=0.99)
...
def test_one_sided_specs():
    up = spyc.capability_analysis(X, usl=USL)
    ...
    lo = spyc.capability_analysis(X, lsl=LSL)
...
def test_subgroup_data_uses_pooled_by_default_and_rbar_option(rng):
    ...
    r = spyc.capability_analysis(g, LSL, USL)
    sp = math.sqrt(np.mean(g.var(axis=1, ddof=1)))
    assert r.within_method == "pooled"
    assert r.sigma_within == pytest.approx(sp / spyc.c4(121))  # c4(sum(n_i - 1) + 1)
    rb = spyc.capability_analysis(g, LSL, USL, within_method="rbar")
    assert rb.sigma_within == pytest.approx(np.mean(g.max(1) - g.min(1)) / spyc.d2(5))
    same = spyc.capability_analysis(g.ravel(), LSL, USL, subgroup_size=5)
    assert same.sigma_within == pytest.approx(r.sigma_within)
    assert r.n == 150


def test_user_supplied_sigma_within():
    r = spyc.capability_analysis(X, LSL, USL, sigma_within=2.0)
    assert r.cp == pytest.approx(10 / 12) and r.within_method == "especificada"


def test_input_validation():
    with pytest.raises(ValueError):
        spyc.capability_analysis(X)
    with pytest.raises(ValueError):
        spyc.capability_analysis(X, 105, 95)
    with pytest.raises(ValueError):
        spyc.capability_analysis(np.full(10, 3.0), 0, 6)
    with pytest.raises(ValueError):
        spyc.capability_analysis(X, LSL, USL, within_method="rbar")  # inválido para individuales
    with pytest.raises(ValueError):
        spyc.capability_analysis(X, LSL, USL, ci_level=1.5)
...
def test_nonnormal_percentile_method_on_weibull():
    ...
    r = spyc.capability_nonnormal(w, lsl=0.05, usl=12, distribution="weibull")
...
def test_nonnormal_normal_dist_agrees_with_normal_pp(rng):
    ...
    nn = spyc.capability_nonnormal(x, 42, 58, distribution="normal")
    nm = spyc.capability_analysis(x, 42, 58)
...
def test_nonnormal_validation():
    with pytest.raises(ValueError):
        spyc.capability_nonnormal([1, 2, -3, 4], 0, 5, distribution="weibull")
    with pytest.raises(ValueError):
        spyc.capability_nonnormal(X, LSL, USL, distribution="cauchy")
...
def test_all_distributions_fit(dist, rng):
    r = spyc.capability_nonnormal(rng.gamma(4, 2, 300) + 0.5, 0.2, 40, distribution=dist)
...
def test_boxcox_recovers_lognormal(rng):
    ...
    r = spyc.capability_boxcox(x, lsl=0.5, usl=9)
    ...
    fixed = spyc.capability_boxcox(x, 0.5, 9, lam=0.0)
    assert fixed.transform["lambda"] == 0.0
    # lambda = 0 equivale a analizar log(x) con log(specs)
    ref = spyc.capability_analysis(np.log(x), math.log(0.5), math.log(9))
...
def test_boxcox_rejects_nonpositive():
    with pytest.raises(ValueError):
        spyc.capability_boxcox([1, 2, 0, 3], 0.5, 5)
    with pytest.raises(ValueError):
        spyc.capability_boxcox(X, lsl=-1, usl=200)
