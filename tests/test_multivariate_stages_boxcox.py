...
import pccpy
from pccpy.multivariate import _boxcox_transform, _t2_reference
...
def test_t2_historicos_con_etapas_usa_los_mismos_limites(rng):
    ...
    ch = pccpy.t2_chart(X, mu=mu, cov=cov, n_hist=100, stages=st)
    ...
    ch2 = pccpy.t2_chart(X, mu=mu, cov=cov, stages=st)
    assert [p["fase"] for p in ch2.params] == ["parámetros conocidos"] * 2
    assert ch2["T2"].ucl[0] == pytest.approx(stats.chi2.ppf(1 - pccpy.multivariate.ALPHA, 3))
...
def test_t2_contribuciones_usan_la_etapa_del_punto(rng):
    ...
    manual = pccpy.t2_chart(X[40:]).contributions(1)
...
def test_t2_etapa_con_pocos_puntos_falla_igual_que_sin_etapas(rng):
    ...
    with pytest.raises(ValueError, match="más de 4"):
        pccpy.t2_chart(X, stages=[1] * 4 + [2] * 76)


def test_t2_dataframe_con_etapas_conserva_nombres(rng):
    X, st = _dos_etapas(rng)
    df = pd.DataFrame(X, columns=["a", "b", "c"])
    ch = pccpy.t2_chart(df, stages=st)
...
def test_gv_con_etapas_reestima_sbar_por_etapa(rng):
    ...
    G = np.vstack([G1, G2])
    ch = pccpy.generalized_variance_chart(G, stages=[1] * 25 + [2] * 25)
    assert ch.params[0]["|S|"] < ch.params[1]["|S|"]  # SIG2 tiene mayor determinante
    solo1 = pccpy.generalized_variance_chart(G1)
...
def test_gv_con_cov_conocida_y_etapas_usa_la_misma_en_todas(rng):
    ...
    G2 = rng.multivariate_normal([0, 0, 0], SIG2, 200).reshape(25, 8, 3)
    G = np.vstack([G1, G2])
    ch = pccpy.generalized_variance_chart(G, cov=SIG1, stages=[1] * 25 + [2] * 25)
...
def test_mewma_reinicia_el_acumulador_en_cada_etapa(rng):
    ...
    ch = pccpy.mewma_chart(X, stages=st, mu=np.zeros(3), cov=SIG1)
    # el primer punto de la etapa 2 se calcula como si Z volviera a 0
    solo2 = pccpy.mewma_chart(X[40:], mu=np.zeros(3), cov=SIG1, ucl=ch.params[0]["LCS"])
...
def test_mewma_fase1_con_etapas_reestima_media_y_cov(rng):
    X, st = _dos_etapas(rng)
    ch = pccpy.mewma_chart(X, stages=st)
    solo1 = pccpy.mewma_chart(X[:40], ucl=ch.params[0]["LCS"])
...
def test_mcusum_reinicia_el_acumulador_en_cada_etapa(rng):
    X, st = _dos_etapas(rng, mu1=(0, 0, 0), mu2=(5, 5, 5))
    ch = pccpy.mcusum_chart(X, stages=st, mu=np.zeros(3), cov=SIG1)
    solo2 = pccpy.mcusum_chart(X[40:], mu=np.zeros(3), cov=SIG1, h=ch.params[0]["LCS"])
...
def test_mcusum_fase1_con_etapas_reestima_media_y_cov(rng):
    X, st = _dos_etapas(rng)
    ch = pccpy.mcusum_chart(X, stages=st)
    solo1 = pccpy.mcusum_chart(X[:40], h=ch.params[0]["LCS"])
...
def test_gv_boxcox_reporta_lambda_y_normaliza(rng):
    ...
    ch = pccpy.generalized_variance_chart(X, boxcox=True)
    assert set(ch.params[0]["lambda_boxcox"]) == {"X1", "X2"}
    transformado, _ = _boxcox_transform(X)
    sin_transformar = pccpy.generalized_variance_chart(transformado)
...
def test_boxcox_conserva_nombres_de_dataframe(rng):
    ...
    ch = pccpy.t2_chart(X, boxcox=True)
...
def test_boxcox_rechaza_combinarse_con_historicos(rng):
    ...
    with pytest.raises(ValueError, match="no se puede combinar"):
        pccpy.t2_chart(X, mu=np.zeros(2), cov=np.eye(2), boxcox=True)
    with pytest.raises(ValueError, match="no se puede combinar"):
        pccpy.mewma_chart(X, mu=np.zeros(2), cov=np.eye(2), boxcox=True)
    with pytest.raises(ValueError, match="no se puede combinar"):
        pccpy.mcusum_chart(X, mu=np.zeros(2), cov=np.eye(2), boxcox=True)
    with pytest.raises(ValueError, match="no se puede combinar"):
        pccpy.generalized_variance_chart(X, subgroup_size=4, cov=np.eye(2), boxcox=True)
...
def test_graficos_con_etapas_y_boxcox(rng):
    X, st = _dos_etapas(rng)
    charts = [
        pccpy.t2_chart(X, stages=st),
        pccpy.generalized_variance_chart(
            rng.multivariate_normal([0, 0, 0], SIG1, 200).reshape(25, 8, 3), stages=[1] * 12 + [2] * 13
        ),
        pccpy.mewma_chart(X, stages=st),
        pccpy.mcusum_chart(X, stages=st),
        pccpy.t2_chart(np.exp(rng.normal(0, 0.3, (50, 3))), boxcox=True),
    ]
    for ch in charts:
        fig = ch.plot()
