...
import pccpy
from pccpy.multivariate import ALPHA, _gv_constants, _mewma_arl, _t2_reference
...
def test_t2_subgrupos_igual_a_n_por_mahalanobis(rng):
    ...
    ch2 = pccpy.t2_chart(G.reshape(60, 3), subgroup_size=4)
...
def test_t2_fase2_con_historicos_detecta_y_diagnostica(rng):
    ...
    X[25:, 2] += 6.0
    ch = pccpy.t2_chart(X, mu=hist.mean(axis=0), cov=np.cov(hist, rowvar=False), n_hist=200)
...
def test_contribuciones_exactas_con_covarianza_identidad():
    ...
    ch = pccpy.t2_chart(X, mu=mu, cov=cov)
...
def test_gv_lineas_manuales_y_deteccion(rng):
    ...
    ch = pccpy.generalized_variance_chart(G)
    ...
    # con Sigma conocida
    ch2 = pccpy.generalized_variance_chart(G, cov=SIGMA)
...
def test_mewma_arl_por_simulacion(rng):
    p, lam, h, runs = 3, 0.2, pccpy.mewma_limit(3, 0.2, 100), 4000
    z = np.zeros((runs, p))
...
def test_mewma_valores_manuales(rng):
    ...
    lam = 0.1
    ch = pccpy.mewma_chart(X, weight=lam, mu=np.zeros(2), cov=np.eye(2), ucl=8.64)
...
def test_mewma_detecta_cambio_pequeno_sostenido(rng):
    ...
    assert not pccpy.mewma_chart(X, mu=mu, cov=cov).in_control


def test_mewma_subgrupos_y_errores(rng):
    X = rng.multivariate_normal([0, 0], np.eye(2), 60)
    ch = pccpy.mewma_chart(X, subgroup_size=3)
    assert ch.params[0]["puntos"] == 20 and ch.params[0]["tamaño"] == 3
    with pytest.raises(ValueError):
        pccpy.mewma_chart(X, weight=0)
    with pytest.raises(ValueError):
        pccpy.mewma_limit(2, 0.1, 1.0)
...
def test_graficos_de_todas_las_cartas_nuevas(rng):
    ...
    charts = [
        pccpy.t2_chart(X),
        pccpy.generalized_variance_chart(X, subgroup_size=4),
        pccpy.mewma_chart(X),
        pccpy.ma_chart(rng.normal(0, 1, 40), length=3),
        pccpy.zmr_chart(rng.normal(0, 1, 18), list("AAAAAABBBBBBAAAAAA")),
        pccpy.imr_rs_chart(rng.normal(0, 1, (20, 4))),
        pccpy.g_chart(rng.geometric(0.03, 40) - 1),
        pccpy.t_chart(rng.exponential(5, 40)),
    ]
    for ch in charts:
        fig = ch.plot()
...
def test_mewma_chart_extremo_a_extremo_tiene_el_arl_pedido(rng):
    ...
    largos = []
    for _ in range(400):
        ch = pccpy.mewma_chart(rng.standard_normal((2500, 2)), mu=mu, cov=cov, arl=100)
        f = ch["MEWMA"].flagged
