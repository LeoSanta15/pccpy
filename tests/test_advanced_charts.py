...
import pccpy
from pccpy.charts.advanced import _geom_quantile, _weibull_mle


# ------------------------------------------------------------------------------ MA
def test_ma_valores_y_limites_manuales(rng):
    x = rng.normal(10, 2, 30)
    ch = pccpy.ma_chart(x, length=4)
    pan = ch["MA"]
...
def test_ma_con_subgrupos_usa_sigma_sobre_raiz_n(rng):
    ...
    ch = pccpy.ma_chart(d, length=3, sigma=1.0, mu=50.0)
...
def test_ma_detecta_desplazamiento_y_valida_length(rng):
    ...
    assert not pccpy.ma_chart(x, length=5, mu=0, sigma=1).in_control
    with pytest.raises(ValueError):
        pccpy.ma_chart(x, length=1)
...
def test_zmr_by_part_y_by_run(rng):
    x, parts = _datos_zmr(rng)
    ch = pccpy.zmr_chart(x, parts, sigma_method="by_part")
    ...
    ch2 = pccpy.zmr_chart(x, parts, sigma_method="by_run")
...
def test_zmr_relative_usa_logaritmo(rng):
    ...
    parts = np.array(list("AAAAAABBBBBBAAAAAA"))
    ch = pccpy.zmr_chart(x, parts, sigma_method="relative")
    y = np.log(x)
    ...
    with pytest.raises(ValueError):
        pccpy.zmr_chart(-x, parts, sigma_method="relative")


def test_zmr_valores_historicos_y_errores(rng):
    x, parts = _datos_zmr(rng)
    ch = pccpy.zmr_chart(x, parts, mu={"A": 50.0, "B": 80.0}, sigma=1.5)
    ...
    with pytest.raises(ValueError, match="Falta la media"):
        pccpy.zmr_chart(x, parts, mu={"A": 50.0})
    with pytest.raises(ValueError, match="una etiqueta por observación"):
        pccpy.zmr_chart(x, parts[:-1])
    with pytest.raises(ValueError):
        pccpy.zmr_chart([1.0, 2, 3, 4], list("ABAB"), sigma_method="by_run")  # corridas de 1 dato


def test_zmr_detecta_valor_extremo(rng):
    x, parts = _datos_zmr(rng)
    x[3] += 25
    assert not pccpy.zmr_chart(x, parts, mu={"A": 50.0, "B": 80.0}, sigma=1.5).in_control


# ---------------------------------------------------------------------- I-MR-R/S
def test_imr_rs_limites_manuales(rng):
    d = rng.normal(20, 1, (25, 5)) + rng.normal(0, 1.5, (25, 1))
    ch = pccpy.imr_rs_chart(d)
    means = d.mean(axis=1)
    ...
    assert ch["R"].ucl == pytest.approx(pccpy.xbar_r_chart(d)["R"].ucl)
    assert ch["R"].center == pytest.approx(pccpy.xbar_r_chart(d)["R"].center)


def test_imr_rs_variacion_entre_no_da_falsas_alarmas(rng):
    d = rng.normal(0, 1, (60, 4)) + rng.normal(0, 4, (60, 1))  # entre >> dentro
    assert pccpy.xbar_r_chart(d, tests=(1,)).violations().shape[0] > 5  # Xbar-R alarma
    assert pccpy.imr_rs_chart(d)["I"].flagged.size <= 2


def test_imr_rs_recupera_sigmas_entre_y_dentro(rng):
    d = rng.normal(0, 1.0, (4000, 5)) + rng.normal(0, 2.0, (4000, 1))
    prm = pccpy.imr_rs_chart(d).params[0]
...
def test_imr_rs_variante_s_y_moda(rng):
    ...
    ch = pccpy.imr_rs_chart(d, within="s", sigma_method="pooled")
    assert ch.kind == "I-MR-S"
    assert ch["S"].ucl == pytest.approx(pccpy.xbar_s_chart(d, sigma_method="pooled")["S"].ucl)
    ...
    prm = pccpy.imr_rs_chart(v, subgroup=ids).params[0]
...
def test_imr_rs_historicos_y_errores(rng):
    ...
    ch = pccpy.imr_rs_chart(d, mu=10.0, sigma_within=1.0, sigma_between=2.0)
    assert ch["I"].ucl[0] == pytest.approx(10 + 3 * np.sqrt(4 + 1 / 5))
    with pytest.raises(ValueError):
        pccpy.imr_rs_chart(d, within="x")
    with pytest.raises(ValueError):
        pccpy.imr_rs_chart(d, within="r", sigma_method="sbar")
...
def test_ewma_descripcion_refleja_k(rng):
    ...
    ch = pccpy.ewma_chart(x, k=2.5, target=0.0, sigma=1.0)
